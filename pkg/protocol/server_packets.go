...
func (p *SPacketClickWindow) Pull(reader buffer.B) error {
    ...
    p.Mode = uint8(reader.PullVarInt())
    p.ClickedItem.IsPresent = reader.PullBool()
    if p.ClickedItem.IsPresent {
        p.ClickedItem.ItemID = int(reader.PullVarInt())
        p.ClickedItem.ItemCount = int(reader.PullByte())
    }
    return nil
}
...
func (p *SPacketEntityAction) Pull(reader buffer.B) error {
...
func (p *SPacketHeldItemChange) Pull(reader buffer.B) error {
...
func (p *SPacketAnimation) Pull(reader buffer.B) error {
