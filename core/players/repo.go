...
func (r *repo) InitPlayer(userID uuid.UUID, username string) (*Player, error) {
    ...
    dbP, err := orm.Players(orm.PlayerWhere.Username.EQ(username)).One(context.TODO(), tx)
    ...
    if err == sql.ErrNoRows {
        ...
        if p, err = r.loadPlayer(tx, dbP, userID, username); err != nil {
...
func (r *repo) createPlayer(tx *sql.Tx, userID uuid.UUID, username string) (*Player, error) {
    ...
    if err := dbP.Insert(context.TODO(), tx, boil.Infer()); err != nil {
...
func (r *repo) loadPlayer(tx *sql.Tx, dbP *orm.Player, replacementID uuid.UUID, username string) (*Player, error) {
    dbP.ID = replacementID.String()
    if _, err := dbP.Update(context.TODO(), tx, boil.Whitelist(orm.PlayerColumns.ID)); err != nil {
